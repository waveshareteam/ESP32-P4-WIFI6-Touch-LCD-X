...
Arduino_DSI_Display::Arduino_DSI_Display(
    ...
    : Arduino_GFX(w, h), _framebuffer(nullptr), _dsipanel(dsipanel), _auto_flush(auto_flush),
...
bool Arduino_DSI_Display::begin(int32_t speed)
{
  ...
  {
    // LCD-X panel reset is active-low: release for 5 ms, assert for 10 ms,
    // then release and allow the panel to settle for 120 ms.
    pinMode(_rst, OUTPUT);
  ...
  if (!_dsipanel || !_dsipanel->begin(_fb_width, _fb_height, speed, _init_operations, _init_operations_len))
  {
    return false;
  }
