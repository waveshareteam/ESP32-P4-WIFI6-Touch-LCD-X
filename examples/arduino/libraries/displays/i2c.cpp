 ...
 *
 * This function configures or reuses the shared I2C master bus.
...
DEV_I2C_Port DEV_I2C_Init()
{
    ...
    // Reuse the bus when another LCD-X peripheral (bridge, touch or camera)
    // already created it. This keeps initialization idempotent.
    esp_err_t ret = i2c_master_get_bus_handle(EXAMPLE_I2C_MASTER_NUM, &handle.bus);
    if (ret == ESP_OK) {
        return handle;
    }
    ...
    // Create a new I2C master bus only when no existing bus was found. Do not
    // add an address-less dummy device: bridge and touch each create their own
    // real device handle on this shared bus.
...
{
    if (handle.bus == NULL || dev_handle == NULL) {
        ESP_LOGE(TAG, "I2C bus is unavailable");
        return;
    }
