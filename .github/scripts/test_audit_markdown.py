...
SCRIPT_PATH = Path(__file__).resolve().parent / "audit_markdown.py"
...
"""
    ...
    def test_repository_config_is_narrow_and_loadable(self) -> None:
        path = REPOSITORY_ROOT / ".github" / "markdown-audit-config.json"
        ...
        homepage_pair = raw["homepage_pairs"][0]
        self.assertEqual("multi-product-hub", homepage_pair["profile"])
        required_components = [
            "centered_header",
            "html_h1",
            "subtitle",
        ...
        self.assertIn("hero_image", required_components)
        self.assertEqual(required_components, homepage_pair["required_components"])
        required_quick_links = [
            "product",
            "documentation",
            "firmware",
        ...
        self.assertIn("product", required_quick_links)
        self.assertEqual(required_quick_links, homepage_pair["required_quick_links"])

        hero_path = "assets/ESP32-P4-WIFI6-Touch-LCD-X.jpg"
        product_url = "https://www.waveshare.com/esp32-p4-wifi6-touch-lcd-7-8-10.1.htm"
        self.assertTrue((REPOSITORY_ROOT / hero_path).is_file(), hero_path)
        for readme_path, product_label, alt_fragment in (
            ("README.md", "Product Page", "product family"),
            ("README_ZH.md", "产品页面", "产品系列"),
        ):
            header = (REPOSITORY_ROOT / readme_path).read_text(encoding="utf-8").split(
                "</div>", 1
            )[0]
            hero_tags = [
                tag
                for tag in re.findall(r"<img\b[^>]*>", header)
                if re.search(rf'\bsrc="{re.escape(hero_path)}"', tag)
            ]
            self.assertEqual(1, len(hero_tags), readme_path)
            alt_match = re.search(r'\balt="([^"]*)"', hero_tags[0])
            self.assertIsNotNone(alt_match, readme_path)
            self.assertTrue(alt_match.group(1).strip(), readme_path)
            self.assertIn(alt_fragment, alt_match.group(1), readme_path)
            product_links = re.findall(
                rf'<a\s+href="([^"]+)"[^>]*>🌐\s*{re.escape(product_label)}</a>', header
            )
            self.assertEqual([product_url], product_links, readme_path)

        config = audit_markdown.load_config(path)
        self.assertEqual(
            "embedded_upstream",
            audit_markdown.classify(
                "examples/esp-idf/10_mp4_player/components/esp_extractor/README.md", config
            ),
        )
        self.assertEqual(
            "unknown",
            audit_markdown.classify("examples/esp-idf/10_mp4_player/components/other/README.md", config),
        )

    def test_repository_docs_workflow_invokes_complete_audit(self) -> None:
        workflow = (REPOSITORY_ROOT / ".github" / "workflows" / "docs.yml").read_text(
            encoding="utf-8"
        )
        self.assertIn("test_audit_markdown.py", workflow)
        self.assertIn("--config .github/markdown-audit-config.json", workflow)
        self.assertIn("--base", workflow)
        self.assertIn("--all --strict", workflow)
