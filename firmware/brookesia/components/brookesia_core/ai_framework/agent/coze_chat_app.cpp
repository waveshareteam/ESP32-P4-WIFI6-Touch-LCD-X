...
void CozeChatAgentInfo::dump() const
{
    ESP_UTILS_LOGI(
        ...
        "\t-private_key_set: %s\n"
        "\t-custom_consumer: %s\n",
        session_name.c_str(), device_id.c_str(), app_id.c_str(), user_id.c_str(),
        static_cast<unsigned>(public_key.size()), private_key.empty() ? "false" : "true",
        custom_consumer.c_str()
    );
}
...
{
    // Build the JWT payload used by the Coze OAuth token exchange.
    ...
    ESP_UTILS_LOGD("payload_str length: %u\n", static_cast<unsigned>(strlen(payload_str)));
    char *formatted_payload_str = cJSON_Print(payload_json);
    if (formatted_payload_str) {
        ESP_UTILS_LOGD("formatted_payload_str length: %u\n", static_cast<unsigned>(strlen(formatted_payload_str)));
    ...
    snprintf(authorization, strlen(jwt) + 16, "Bearer %s", jwt);
    ESP_UTILS_LOGD("Authorization header prepared");
    ...
    if (response.body) {
        ESP_UTILS_LOGD("response body length: %u\n", static_cast<unsigned>(strlen(response.body)));
        ...
        if (root) {
            ...
            if (cJSON_IsString(access_token_item) && access_token_item->valuestring != NULL) {
                ESP_UTILS_LOGD(
                    "access_token received, len: %u\n",
                    static_cast<unsigned>(strlen(access_token_item->valuestring))
                );
    ...
    // Release transient request objects after the token has been copied out.
    free(jwt);
    free(authorization);
    cJSON_Delete(http_req_json);
