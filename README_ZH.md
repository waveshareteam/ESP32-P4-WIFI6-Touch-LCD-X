...
[`examples/arduino/`](examples/arduino/) 提供两个第一方示例：
[`01_DisplayColorBars`](examples/arduino/examples/01_DisplayColorBars/) 和
[`02_TouchDrawing`](examples/arduino/examples/02_TouchDrawing/)。两个示例均使用
随仓库提供的 `lcd_x` 库，并支持 LCD-7、LCD-8 和 LCD-10.1。完整的
...
使用 Arduino CLI `1.5.1` 和 Arduino-ESP32 `3.3.11` 为两个 Arduino 示例的全部
三种显示型号编译（共 6 行）。该工作流仅编译，不发布固件或可下载构建产物。
